...
Кортежи - это те же списки, но без возможности их изменить. Возникает логичный вопрос, зачем они нужны? В большинстве случаев они необходимы для хранения данных, которые гарантированно не будут изменяться во время работы программы (защита от дурака).

Работа с кортежами аналогична работе со списками:
...
>>> planets[-1]
'Mars'
>>> stuff = (1, 3.14, 'test', [1, 2, 3])
>>> stuff.index(3.14)
1
```

К кортежу применимы все те же функции, как и к списку (за исключением функций, которые изменяют сам кортеж).

Интересные ссылки по кортежам [[5]](https://younglinux.info/python/tuple) [[6]](https://pythonworld.ru/tipy-dannyx-v-python/kortezhi-tuple.html).

### Словари

В программировании существует концепция, называемая хэш-таблица (только в `python` это называется словарь). Словарь - структура данных для хренения информации в виде пары ключ-значение.

К примеру, нам необходимо решить задачу о хранении оценок студентов. Мы могли бы использовать список учеников и список оценок:

```python
>>> students = ["Иван", "Люся", "Галя"]
>>> grades = [[7, 5, 4], [4, 2], [9, 9, 10]]
>>> grades[students.index("Люся")]  # Получить оценки Люси
[4, 2]
```

Однако при таком подходе есть множество недостатков. Сможете их описать?

Вместо решения, описанного выше, мы можем использовать более подходящую структуру для хранения наших данных - словарь.

```python
>>> students = {"Иван": [7, 5, 4], "Люся": [4, 2], "Галя": [9, 9, 10]}
>>> students["Иван"]  # Получить оценки Ивана
[7, 5, 4]
>>> students["Люся"].append(9)  # Люсе сегодня фортануло (наверное - литература)
>>> students["Люся"]
[4, 2, 9]
```

Как говорилось выше, словари - это хранение данных по типу ключ-значение (или ещё одно название - ассоциативный массив, так как каждому значение стоит своя ассоциация - ключ).

Ключи словаря должны быть уникальными, иначе, будет взято только последнее указанное значение:
...
Мы так же имеем множество различных полезных функций для работы со словарями:

```python
>>> my_dict = dict(key='value', PI=3.14)
>>> my_dict
{'key': 'value', 'PI': 3.14}
>>> my_dict.keys()  # Получить список ключей
dict_keys(['key', 'PI'])
>>> my_dict.values()  # Получить список значений
dict_values(['value', 3.14])
>>> my_dict.pop('PI')  # Удалить пару ключ-значение из словаря и вернуть их
3.14
>>> my_dict
{'key': 'value'}
>>> my_dict['new_key'] = 'value'  # Добавить новую пару ключ-значение
>>> my_dict
{'key': 'value', 'new_key': 'value'}
```

 Полезные ссылки по словарям [[7]](https://pythonworld.ru/tipy-dannyx-v-python/slovari-dict-funkcii-i-metody-slovarej.html).

### Множества

Множества - структура данных для хранения уникальных элементов в случайном порядке.

Определяется множество очень схоже со списками и кортежами, но вместо квадратных и круглых скобок соответственно используются фигурные:
...
Множество поддерживает различные математические операции - пересечение, объединение и т.д.
...
Полезные ссылки про можества [[8]](https://pythonworld.ru/tipy-dannyx-v-python/mnozhestva-set-i-frozenset.html).
...
[5] [Кортежи и примеры](https://younglinux.info/python/tuple)

[6] [Ещё немного о кортежах](https://pythonworld.ru/tipy-dannyx-v-python/kortezhi-tuple.html)

[7] [Функции словарей](https://pythonworld.ru/tipy-dannyx-v-python/slovari-dict-funkcii-i-metody-slovarej.html)

[8] [Функции множества](https://pythonworld.ru/tipy-dannyx-v-python/mnozhestva-set-i-frozenset.html)
